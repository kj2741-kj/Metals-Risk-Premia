"""
regime_r1_aluminium.py -- R1 Term-Structure Regime Portfolio, Aluminium

Mirrors regime_r1_v2.py (Copper) but on Aluminium's own production legs:
  Momentum: MA(60,115)              (unchanged production default, genuine gap)
  Carry:    Z-score-252d            (already Aluminium's production carry leg --
                                      no substitution needed, unlike Copper where
                                      CarryMom-20d had to be swapped out for the
                                      regime study because it shares a data-generating
                                      variable with the roll_yield-level regime classifier)
  Value:    V1 F12, 5yr, +/-10%

Regime = sign of roll_yield = (F1-F2)/F1, shift-1 (no look-ahead, not estimated).
Weights are IS-fit per walk-forward window (QP max-Sharpe, >=0, sum=1), separately
for backwardation and contango days within each 5yr IS window -- identical
methodology to the Copper R1 v2 test, for a clean like-for-like comparison.
"""
...
al_sh = "ALuminium LME"
df_raw = pd.read_excel(xls, sheet_name=al_sh, header=None, nrows=5)
...
df_c = pd.read_excel(xls, sheet_name=al_sh, header=hr if hr else [0, 1])
...
print(f"Aluminium F1: {len(f1r)} rows | Curve: {len(crv)} rows | {crv.index[0].date()} -> {crv.index[-1].date()}")

# ── Legs (Aluminium production config) ─────────────────────────────────────────
mom_pos = np.sign(f1r.rolling(60).mean() - f1r.rolling(115).mean()).shift(1).fillna(0)
...
print("=" * 112)
print("R1 ALUMINIUM -- IS-OPTIMISED REGIME WEIGHTS (Mom MA(60,115) / Carry Z-score-252d / Value V1 F12), net 5bps")
print("=" * 112)
hdr = (f"{'OOS':7s} {'%BD':>6s} | {'w_BD (M/C/V)':>16s} | {'w_CTG (M/C/V)':>16s} | {'EW':>7s} {'R1':>7s} {'Diff':>7s}")
print(hdr)
print("-" * 90)
...
while oos_s < T:
    ...
    port_r1 = pd.Series(w_oos[:,0]*m_oos + w_oos[:,1]*c_oos + w_oos[:,2]*v_oos, index=oos_dt)
    ...
    sh_r1 = win_sharpe_net(port_r1)
    bd_pct = 100 * bd_oos.mean()

    print(f"{oos_yr:7s} {bd_pct:5.1f}% | {w_bd[0]:.2f}/{w_bd[1]:.2f}/{w_bd[2]:.2f}    | "
          f"{w_ctg[0]:.2f}/{w_ctg[1]:.2f}/{w_ctg[2]:.2f}    | {sh_ew:+7.3f} {sh_r1:+7.3f} {sh_r1-sh_ew:+7.3f}")
...
print("-" * 90)
avg_ew, avg_r1 = np.nanmean(df["EW"]), np.nanmean(df["R1"])
print(f"AVERAGE OOS SHARPE:  EW={avg_ew:+.3f}   R1={avg_r1:+.3f}   Diff={avg_r1-avg_ew:+.3f}")
post22 = df[df["OOS"].str.replace("*","").astype(int) >= 2022]
print(f"POST-2022 AVERAGE:   EW={np.nanmean(post22['EW']):+.3f}   R1={np.nanmean(post22['R1']):+.3f}   "
      f"Diff={np.nanmean(post22['R1'])-np.nanmean(post22['EW']):+.3f}")
