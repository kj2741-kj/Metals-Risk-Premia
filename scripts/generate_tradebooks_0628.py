"""
generate_tradebooks_0628.py  — F1_raw-TC variant (2026-06-28)
=================================================================
Folder: "Tradebooks 6-28/"  (same internal structure as tradebooks/)
Identical to generate_tradebooks.py EXCEPT the transaction cost is charged on the
actual traded front-month price F1_raw (not the back-adjusted continuous index).
Gross P&L is unchanged; only the Net (after-TC) columns differ — slightly.

TC incorporated in EVERY tradebook:
  TC_Cost(t) = |Position(t) − Position(t−1)| × (5/10,000/2) × F1_raw(t)
  Net_PnL(t) = Gross_PnL(t) − TC_Cost(t)
  Net_Return(t) = Net_PnL(t) / F1_Continuous(t−1)     [returns still in continuous space]
  ...
  This matches the dashboard's F1_raw-TC formula (tc_bps/10000/2 on F1_raw).
...
BASE    = DATA_DIR
TBDIR   = os.path.join(OUTPUTS_DIR, "Tradebooks 6-28")   # F1_raw-TC variant (2026-06-28)
...
TC_DESC_ROWS = [
    ...
    ("Convention",
     "Matches the dashboard's TC formula: pnl -= chg × (tc_bps/10000/2) × F1_raw(t) — the spread is "
     "paid on the actual traded front-month price (F1_raw), not the back-adjusted continuous index. "
     "PnL/returns are still computed on F1_continuous. TC columns are shown in BLUE in the Tradebook sheet."),
    ("Net Sharpe",
...
def _tail(pos_raw: pd.Series, f1c_s: pd.Series, tc_bps: int = 5):
    ...
    tc  = (chg * (tc_bps / 10000.0 / 2.0) * f1r.reindex(idx)).where(~warm)   # TC on F1_raw
    net = gp - tc
    with np.errstate(invalid="ignore", divide="ignore"):
